...
# def resolve_angle_specs(
#     *,
#     quat: Sequence[Number] | None = None,
#     axisangle: Sequence[Number] | None = None,
#     xyaxes: Sequence[Number] | None = None,
#     zaxis: Sequence[Number] | None = None,
#     euler: Sequence[Number] | None = None,
#     default: tuple[str, tuple[float, ...]] = ("quat", (1, 0, 0, 0)),
...
#         exp_dim = expected_dims[key]
#         if not (
#             isinstance(value, Sequence)
#             and len(value) == exp_dim
#             and all(isinstance(x, Number) for x in value)
#         ):
#             raise ValueError(f"Invalid {key}: must be {exp_dim} numbers, got {value}.")
#         parsed[key] = tuple(value)
#
#     if (n_specified := sum(v is not None for v in parsed.values())) == 0:
#         return default
#     elif n_specified == 1:
#         return [(k, v) for k, v in parsed.items() if v is not None][0]
#     else:
#         raise ValueError(f"Cannot specify more than one of {parsed.keys()}.")
