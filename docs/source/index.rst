...
The environments in this package serve as wrappers to provide a unified interface to interact with fly model in different physics simulators (PyBullet, MuJoCo, Isaac Gym).
...
- The XML file defining the MuJoCo model is modified from an XML file generated by `FARMS`_ (particularly `farms_mujoco`_).
