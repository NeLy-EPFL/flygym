...
def advancement_transfer(phases, step_dur, match_leg_to_joints):
    ...
    t_indices = np.round(np.mod(phases*step_dur/CPG_PERIOD, step_dur-1)).astype(int)
    t_indices = t_indices[match_leg_to_joints]
    
    return t_indices


def phase_oscillator(_time, state, n_oscillators, frequencies, coupling_weights, phase_biases, target_amplitudes, rates):
...
def initialize_solver(int_f, integ_name, t, n_oscillators, frequencies, coupling_weights, phase_biases, target_amplitudes, rates, int_params ={}):
    solver = ode(f=int_f)
    solver.set_integrator(integ_name, **int_params)
    initial_values = np.random.rand(2*n_oscillators)
    solver.set_initial_value(y=initial_values, t=t).set_f_params(n_oscillators, frequencies,
                                                                  coupling_weights, phase_biases,
                                                                    target_amplitudes, rates)
    return solver

# From de Angelis et al. 2019 eLife
phase_biases_tripod_measured= np.array([[0, 0.43, 0.86, 0.5, 0.93, 1.36],
...
phase_biases_tripod_idealized = np.array([[0, 0.5, 1.0, 0.5, 1.0, 0.5],
...
phase_biases_metachronal_idealized = np.array([[0, 1/6, 2/6, 3/6, 4/6, 5/6],
...
phase_biases_ltetrapod_idealized = np.array([[0, -1/3, 1/3, 1/3, 1, -1/3],
                                            [1/3, 0, -1/3, -1/3, 1/3, 1],
                                            [-1/3, 1/3, 0, 1, -1/3, 1/3],
                                            [-1/3, 1/3, 1, 0, -1/3, 1/3],
                                            [1, -1/3, 1/3, 1/3, 0, -1/3],
